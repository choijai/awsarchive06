...
## 📋 생성 시 필수 규칙:
1. **goal (핵심 목표)**: 이 문제가 테스트하는 핵심 목표를 한 문장으로 명확히
2. **keywords (핵심 키워드)**: 문제에서 가장 중요한 AWS 개념 3~4개
3. **easyMode 설명**: 초등학교 5학년도 이해하는 비유법 사용
   - 각 선택지를 어린이 수준으로 설명
   - 함정답이 왜 틀렸는지 쉽게 설명
4. **explanation.correct**: 정답이 왜 맞는지 기술적으로 명확하게
5. **explanation.trap_***: 각 함정답이 정확히 어느 부분에서 실패하는지 구체적으로
6. **SAA 시험용 요점**: 시험에서 자주 출제되는 핵심 개념 정리
