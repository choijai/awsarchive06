...
다음 3가지 예시 문제의 스타일, 난이도, 함정답 구조를 정확히 분석하고,
...
정답: B
핵심:
- DataSync: 온프레미스 ↔ AWS 스토리지 서비스 간 안전한 데이터 이동 자동화
- DMS: 데이터베이스 마이그레이션 서비스 (파일 전송에는 부적합) → C, D 오답
- 공용 인터넷: 민감한 데이터 노출 위험 → A 오답
- Direct Connect: 전용 네트워크 연결 (보안 강화)
- 함정답 특징: A/C는 공용 인터넷으로 보안 부족(1가지 미충족), D는 완전히 다른 서비스
...
정답: C
핵심:
- API Gateway: 완전관리형 API 서비스 (EC2 관리 불필요) → A, B는 운영 오버헤드 증가
- Kinesis Data Streams: 실시간 스트리밍 데이터 수집
- Kinesis Data Firehose: 스트림 → S3 자동 전달, 변환 가능
- Lambda: 서버리스 변환
- Glue: ETL 배치 처리 서비스 (실시간 스트리밍에 부적합) → D 오답
- 함정답 특징: A (EC2 오버헤드), B (EC2 + Glue 부적합), D (Glue는 실시간 스트리밍 부적합)
...
정답: B
핵심:
- PITR: 최근 35일만 복구 가능 (7년 불가) → A 오답
- AWS Backup: 중앙 집중식 백업 관리, 콜드 스토리지 자동 전환, 생명주기 관리 → 운영 효율성 최고
- S3 내보내기: 가능하지만 수동 관리 필요 → C는 B보다 운영 복잡
- Lambda + EventBridge: 맞지만 관리 오버헤드 높음 → D는 너무 복잡
- 함정답 특징: A (시간 제한), C (가능하지만 수동), D (불필요한 복잡성)
...
정답: A
핵심:
- 온디맨드 모드: 트래픽 예측 불가능 상황에서 최적, 사용량만 청구 (비용 효율)
- 프로비저닝 + Auto Scaling: 스케일링에 지연 발생, 사용하지 않는 용량도 비용 부과 → C는 급증 대응 지연
- GSI: 쿼리 성능 개선 (비용 최적화 문제 아님) → B는 요구사항 불일치
- Global Table: 고가용성 필요 없음, 추가 비용 → D는 불필요한 복잡성
- 함정답 특징: B (기능은 좋지만 요구사항 불일치), C (비용 최적화 미흡), D (불필요한 고가용성)
...
**새로운 문제의 조건:**
1. 시나리오: 실제 기업 상황 반영, 구체적 수치 포함 (예: "월 100TB 데이터", "초당 5000 요청", "6개월 보관")
2. 제약 조건: 2~3개 복합 조건 (예: "비용 최적화 + 최소 운영 오버헤드", "높은 가용성 + 낮은 지연시간")
3. 선택지 A~D:
   - 각 선택지 3~4줄 (구체적 AWS 서비스 설정 포함)
   - 1개 정답: 모든 조건 충족
   - 3개 함정답: 거의 맞지만 1가지 조건 미충족하는 구조
4. 정답 및 상세 설명:
   - 정답 선택지가 모든 조건을 충족하는 이유
   - 각 함정답이 어느 조건을 미충족하는지 명시
   - AWS 공식 문서 기반 설명
...
JSON 형식으로 응답해주세요:
{
  ...
  "answer": "B",
  "explanation": {
    "correct": "정답인 이유 + AWS 서비스 특징",
    "trap_A": "함정답 A - 어느 조건 미충족",
    "trap_C": "함정답 C - 어느 조건 미충족",
    "trap_D": "함정답 D - 어느 조건 미충족"
  },
  "patterns": ["이 문제의 핵심 패턴 1", "핵심 패턴 2"]
}`;
