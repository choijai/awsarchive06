 ...
 * - 2개 어느 정도 가능한 선택지
 */
export const SAA_PROBLEM_PROMPT_MEDIUM = `당신은 AWS SAA-C03 시험 출제 전문가입니다.
...
JSON 형식으로 응답해주세요 (마크다운 코드 블록 없이 순수 JSON):
{
  ...
  "goal": "이 문제의 핵심 목표 (한 문장)",
  "easyMode": {
    "explanation": "정답에 대한 초등학교 5학년 수준의 설명 (비유법 사용)",
    "A": "선택지 A를 초등학교 5학년 수준으로 설명",
    "B": "선택지 B를 초등학교 5학년 수준으로 설명",
    "C": "선택지 C를 초등학교 5학년 수준으로 설명",
    "D": "선택지 D를 초등학교 5학년 수준으로 설명"
  },
  "explanation": {
...
JSON 형식으로 응답:
{
  ...
  "options": {
    "A": "선택지 A (거의 맞지만 1개 제약 미충족)",
    "B": "선택지 B (거의 맞지만 다른 1개 제약 미충족)",
    "C": "선택지 C (모든 제약 충족)",
    "D": "선택지 D (가능하지만 과도함)"
  },
  "answer": "C",
  ...
  "explanation": {
    "architecture": "2~3개 서비스 조합, 아키텍처 흐름",
    "correct": "각 제약이 어떻게 만족되는지 + 비용/성능 계산",
    "service_features": "각 AWS 서비스의 특징 + 제한사항",
    "trap_A": "어느 제약 미충족 + 왜 그 선택은 안 되는가",
    "trap_B": "어느 제약 미충족 + 구체적 비용/성능 수치",
    "trap_C": "어느 제약 미충족 + 대안 제시"
  },
...
- 선택지: 각 4~5줄, 아키텍처 명시, 보안/비용/성능 모두 포함
...
JSON 형식으로 응답:
{
  "question": "실무 수준 복합 시나리오 (온프레미스 마이그레이션, 멀티 리전, 규제 등)",
  "constraint": ["기술 제약 1", "기술 제약 2", "비즈니스 제약", "운영 제약"],
  "options": {
    "A": "선택지 A (기술은 좋지만 규제/운영 미흡)",
    "B": "선택지 B (거의 맞지만 비용 초과 또는 운영 복잡)",
    "C": "선택지 C (모든 제약 완벽 충족)",
    "D": "선택지 D (과도한 고가용성/성능, 비용 과다, 불필요한 복잡성)"
  },
  "answer": "C",
  "keywords": ["핵심 키워드 1", "핵심 키워드 2", "핵심 키워드 3"],
  "goal": "이 문제의 핵심 목표 (한 문장)",
  ...
  "explanation": {
    "architecture": "3~4개 서비스, 온프레미스 연결, 마이그레이션 경로, 데이터 흐름 상세",
    "correct": "각 제약별 충족 증명 + 왜 이것이 최적 솔루션인가 + 대안과의 비교",
    "service_features": "각 서비스의 용도 + 한계 + 조합 시 시너지",
    "trap_A": "어느 제약 미충족 (예: 규제 요구 미충족) + 구체적 이유",
    "trap_B": "어느 제약 미충족 (예: 운영 복잡도) + 왜 기존 팀이 관리 못하는가",
    "trap_C": "어느 제약 미충족 (예: 비용 초과) + 초과액 계산"
  },
  "patterns": ["핵심 패턴 1", "핵심 패턴 2", "핵심 패턴 3"]
}`;
